...
func configureCli() (app *cli.App) {
	...
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:   "connect-to, c",
			Usage:  "Host & port to connect to, e.g. sandbox.babl.sh:4445, localhost:4444",
			Value:  "sandbox.babl.sh:4445",
			EnvVar: "BABL_ENDPOINT",
		},
		cli.StringFlag{
			Name:   "storage",
			Usage:  "Endpoint for Babl storage",
			Value:  "sandbox.babl.sh:4443",
			EnvVar: "BABL_STORAGE",
		},
		cli.BoolFlag{
			Name:   "async",
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: func(_ *cli.Context) {
				m := bablutils.NewUpgrade("babl", []string{})
				m.Upgrade(Version)
			},
		},
	}
